...
- [数据传输问题](#数据传输问题)
  - [分析方法](#分析方法-6)
    ...
    - [方法：检查是否存在syslog刷屏](#方法检查是否存在syslog刷屏)
    - [方法：通过air log观察数据包传输速率是否过低](#方法通过air-log观察数据包传输速率是否过低)
    - [方法：观察数据包发包间隔是否过大](#方法观察数据包发包间隔是否过大)
  - [典型问题](#典型问题-6)
...
本章介绍Serial Port Profile（SPP）相关问题常用的分析、定位方法。SPP连接建立在ACL已连接的基础之上。在分析SPP连接问题之前，需要先确认ACL连接是否正常建立。
SPP中定义了Device A（DevA）和 Device B（DevB）两个角色。DevA是主动连接方，DevB是被动连接方。在软件实现中，通常对应到Client和Server两个角色。
Server需要指定占用的本端的RFCOMM Channel进行监听，同时需要指定一个UUID描述提供的服务。Server需要通过注册Service Discovery Protocol（SDP）服务来向Client提供服务发现信息。Client需要通过SDP查询Server的服务信息，然后通过RFCOMM Channel信息向对应的Server发起连接请求。
...
* [确认SPP连接状态与断连发起方](#方法：确认SPP连接状态与断连发起方)
  * 若是之前连接尚未断开，则需要确认SPP连接双方是否有发起断连操作。
...
#### 1 通过air log观察当前空口环境是否复杂

可以从图中的粉色柱体看到整个传输过程中的重传率，如下代表信道质量尚可

<img src="img/how_to_analyze_bluetooth_issues/gatt/channel_quality.png" alt="sniff:信道传输质量" width="50%">

<a id="方法：检查是否存在syslog刷屏">

### 方法：检查是否存在syslog刷屏

大量的log刷屏会影响设备处理性能，进而导致数据传输性能下降。请观察在数据传输过程中是否存在大量蓝牙syslog输出。

<a id="方法：通过air log观察数据包传输速率是否过低">

### 方法：通过air log观察数据包传输速率是否过低

在良好的信道环境下，蓝牙支持3M数据包传输，可以通过air log观察到空口中传输的数据包类型，典型log如下：
...
SPP吞吐量过低的问题可能有多种原因导致，应当先通过以下方法初步排查问题：

* [检查是否存在syslog刷屏](#方法：检查是否存在syslog刷屏)
  * 若存在大量syslog刷屏，则需要先在蓝牙工具bttool中输入命令`log disable stack`关闭蓝牙协议栈log与`log disable snoop`关闭snoop工具。
  * 否则，请按照下面的步骤进一步分析。

* [观察当前空口环境是否复杂](#方法：观察当前空口环境是否复杂)
  * 首先建议使用客观屏蔽环境对蓝牙传输性能进行验证评估，排除环境干扰。
  * 若屏蔽环境下，仍然传输性能较差，则需要按照下面的步骤进一步分析。
